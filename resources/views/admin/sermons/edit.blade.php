	...
	<div>
		...
		<form action="{{ route('admin.sermons.update',[$sermon->id]) }}" method="post" enctype="multipart/form-data">
			...
			<div class="my-2">
				...
				<div class="mb-4">
					<label for="category_sermon">Categorie Sermon</label>
					<select class="form-control form-select" name="category_sermon_id" id="category_sermon">
						@foreach ($categorySermons as $item)
							<option {{$item->id==$sermon->category_sermon_id?'selected':''}} value="{{$item->id}}">{{$item->titre}}</option>
						@endforeach
					</select>
				</div>
				...
				<div class="mb-4">
					...
					<textarea name="description" class="form-control" id="" cols="30" rows="5">{{$sermon->description}}</textarea>
				</div>
